...
// min_f以上max_f以上のyだけ通す、他は0にする
void bandpass(complex double * y, long n, long min_f, long max_f, long sampling_f) {
  for (long i = 0; i < n; i++) {
    long f = sampling_f * i / n;
    if (f < min_f || max_f < f) {
      y[i] = 0;
    }
  }
...
int main(int argc, char ** argv) {
  (void)argc;
  if (argc != 4) die("please fill args");
  long n = atol(argv[1]);
  long min_f = atol(argv[2]);
  long max_f = atol(argv[3]);
  long sampling_f = 44100;
  if (!pow2check(n)) {
  ...
  while (1) {
    ...
    bandpass(Y, n, min_f, max_f, sampling_f);
